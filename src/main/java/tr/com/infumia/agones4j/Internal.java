...
final class Internal {
  ...
  static final String GRPC_HOST = Internal.getEnv(Internal.ENV_GRPC_HOST, Internal.DEFAULT_GRPC_HOST);
  static final int GRPC_PORT = Internal.getEnvAsInt(Internal.ENV_GRPC_PORT, Internal.DEFAULT_GRPC_PORT);
  ...
  static AgonesCounter toCounter(final Alpha.Counter counter) {
    return new AgonesCounter(counter.getName(), counter.getCapacity(), counter.getCount());
  }

  static AgonesList toList(final Alpha.List list) {
    return new AgonesList(list.getName(), list.getCapacity(), list.getValuesList());
  }

  static Alpha.List toAgonesList(final AgonesList list) {
    return Alpha.List
      .newBuilder()
  ...
  static <T> StreamObserver<T> observerOnNext(final Executor executor, final List<Consumer<T>> consumers) {
  ...
  static <T, R> StreamObserver<R> observerMap(final StreamObserver<T> observer, final Function<R, T> mapper) {
